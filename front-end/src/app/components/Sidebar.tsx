  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-white/[0.06] flex-shrink-0">
        {!collapsed && (
          <BrandLogo subtitle={userRole === 'admin' ? 'Admin Portal' : 'Kitchen Portal'} />
        )}
        {collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5 scrollbar-hide">
        {navGroups.map((group) => (
          <div key={group.label} className="mb-3">
            {/* Group label */}
            {!collapsed && (
              <p className="text-[10px] font-semibold text-[#E0E1B7]/40 tracking-widest uppercase px-3 py-1.5 mb-1">
                {group.label}
              </p>
            )}
      ...
      <div className="px-2 py-3 border-t border-white/[0.06] space-y-0.5 flex-shrink-0">
        ...
        <Link
          ...
          title={collapsed ? 'العربية' : undefined}
