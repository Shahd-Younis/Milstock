...
import { ProductWarehouseDoc, WarehouseDoc } from '../lib/types';
...
export const WarehouseLocations = () => {
  ...
  const { data: stockRows, loading: stockLoading } =
    useApiResource<ProductWarehouseDoc>(() => api.productWarehouses.list(), []);
  ...
  const warehouseCards = warehouses.map((warehouse) => {
    ...
    return {
      ...warehouse,
      capacity: Math.max(currentStock + 1000, 5000),
      currentStock,
                    ...
                    <div className="pt-4 border-t border-border">
                      ...
                      <div className="space-y-2">
                        {warehouse.categories.length === 0 && (
                          <p className="text-xs text-muted-foreground">No product stock rows yet.</p>
                        )}
