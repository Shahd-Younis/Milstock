...
import { CheckCircle, Clock, XCircle, Package, Truck, User, ArrowLeft } from 'lucide-react';
...
const requestedItems = [
  ...
  { id: 'INV-003', name: 'Water Bottles', quantity: 200, kitchen: 'kitchens', available: 1000 },
];
                ...
                {timelineSteps.map((step, index) => (
                  <div key={index} className="flex gap-4">
                      ...
                      <div
                        className={`p-3 rounded-full ${
                          step.status === 'completed'
                            ? 'bg-[#6A7B4D] text-white'
                            : step.status === 'current'
                            ? 'bg-[#4B5B3A] text-white'
                            : 'bg-[#E0E1B7] text-muted-foreground'
                        }`}
                      >
                        {step.status === 'completed' ? (
                      ...
                      {index !== timelineSteps.length - 1 && (
                        <div
                          className={`w-0.5 h-16 mt-2 ${
                            step.status === 'completed' ? 'bg-[#6A7B4D]' : 'bg-border'
                          }`}
                        />
                      )}
                    </div>
                    <div className="flex-1 pb-8">
                      ...
                      {step.notes && (
                        <div className="mt-3 p-3 bg-[#6A7B4D] bg-opacity-10 rounded-lg">
