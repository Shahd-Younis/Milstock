...
export const NotificationsPage = () => {
  const { data, loading, error } = useApiResource<NotificationDoc>(() => api.notifications.list(), []);
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="space-y-3">
        {!loading && notifications.length === 0 && (
          <div className="bg-white rounded-2xl border border-[#4E4631]/10 p-8 text-center text-sm text-[#5A6B50]">
            {error || 'No MongoDB notifications found. Run npm run seed in the backend.'}
          </div>
        )}
        {notifications.map((notification) => {
          const Icon = getIcon(notification.type);
          const config = severityConfig[getSeverity(notification.type) as keyof typeof severityConfig];
          return (
            <div
              key={notification._id}
              className={`bg-white rounded-2xl border p-5 shadow-sm transition-all hover:shadow-md ${
                !notification.is_read ? `border-l-4 ${config.accent} border-[#4E4631]/10` : 'border-[#4E4631]/10'
              }`}
            >
