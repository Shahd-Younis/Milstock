...
const UserManagement = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return user.name.toLowerCase().includes(search) || user.email.toLowerCase().includes(search) || user.military_number.toLowerCase().includes(search);
  });
  const columns = [
    ...
    { key: "email", header: "Email" },
    {
      key: "role",
      header: "Role",
      render: (row) => <Badge variant={row.role === "admin" ? "info" : "neutral"}>
    ...
    },
    {
      key: "actions",
      header: "Actions",
      render: () => <button className="p-2 hover:bg-[#E0E1B7] rounded-lg transition-colors">
